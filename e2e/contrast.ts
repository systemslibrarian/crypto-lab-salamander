 ...
 *    an unknown backdrop, or a `color-mix()`. This lab's entire verdict palette
 *    is built out of the first kind: `--ok-soft`, `--alarm-soft`, `--warn-soft`
 *    and `--accent-soft` are all `rgba(…, 0.09-0.14)` over whatever panel is
 *    behind them, and they are the fill of every `.verdict`, every
 *    `.result-badge`, every `.pill` and every `.tagcell` — that is, of every
 *    single indicator this lab exists to show. `.block.msg2` adds
 *    `rgba(110, 168, 254, 0.12)` and `.cl-hero-why` a
 *    `color-mix(in oklab, var(--accent) 6%, transparent)`. A violations-only
 *    gate measured the contrast of almost nothing that carries a result here.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Here that is `.noise-canvas`
 *    at `.95` (no text, but it sets the pattern) and, more usefully, the
 *    ABSENCE of the usual case: `#app button.btn:disabled` deliberately sets
 *    `opacity: 1` and reaches for `filter: grayscale(0.4)` and an explicit
 *    `--text-muted` instead, so a disabled control here is a real colour
 *    decision rather than a dimmed one — and the walk measures it as such.
 *
 ...
 * not behind it. Nothing in this lab is positioned today — `style.css` contains
 * no `position` declaration at all outside the shared top bar's `sticky` — so
 * the geometry test is a no-op here, and it stays because the first absolutely
 * positioned tooltip anyone adds would otherwise be measured against a surface
 * it does not sit on. Opacity is unconditional either way — an opacity group
 * fades its whole subtree wherever that subtree happens to paint.
 *
 ...
 *    boxes are the three `.math` blocks (`overflow-x: auto` around 32-hex-char
 *    GF(2^128) field elements) and `.tageq` (`overflow-x: auto` around two rows
 *    of sixteen tag cells). Content scrolled past those is not dimmed or partly
 *    drawn — it is absent from the frame, and asking what colour it sits on has
 *    no answer. Its rect is still outside every ancestor's box, so the ancestor
 ...
 *    draws its two figures on `<canvas>` — `pixels.ts` paints raw RGB into a
 *    `.glyph-canvas` and a `.noise-canvas`, both `role="img"` with an
 *    `aria-label` — so there is no SVG text on the page at all beyond the shared
 *    header's two `aria-hidden` marks. The branch is kept because an
 ...
 *    report a ratio nothing on screen has, the day this lab grows a chart.
 *
 ...
 * not one — nor is it one to axe's `color-contrast` rule. Neither this lab's
 * stylesheet nor the shared header declares any pseudo-element content, which
 * was checked rather than assumed, so the gap is currently empty here. It is
 * covered anyway by `nontext.ts`'s second check, which reads `content` off every
 * pseudo on the page and measures its ink — because "currently empty" is a
 * property of today's stylesheet, not of the page.
 *
 * A SECOND GAP, and this one is not empty: the two `<canvas>` figures. Their
 * pixels are painted by `paintRgb()` from raw forged plaintext and no oracle
 * here reads a canvas bitmap. Both carry an `aria-label` describing what they
 * show, and the noise canvas is deliberately noise, so nothing in them is a
 * MEANINGFUL graphic whose contrast is required — but that is a judgement,
 * written down, rather than something measured.
 */
 ...
 * uses. This lab has exactly one animation, `fade`, run by the `.reveal` class
 * every panel adds when it renders — and it is declared inside
 * `@media (prefers-reduced-motion: no-preference)`, so under the reduced motion
 * this gate asserts it is never declared at all and nothing here paints
 * transiently. There is no half-faded frame for a scan to catch.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives less than a full axe pass would
 * be caught only sometimes, and a sometimes-measured assertion is worse than
 * none. Nothing on this page is in that shape today; the day a status line
 * starts flashing a confirmation, it will be.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks eight panels and — the expensive part — the
     * `.tageq` tag comparison, which renders each 16-byte tag as sixteen
     * individual `.tagcell` spans, twice, all of them re-walking the same
     ...
     * during the pass, so the cached values cannot go stale: every render here
     * is synchronous inside a click handler, and the one deferred render in the
     * lab — `main.ts`'s `autoRun()` — fires 60ms after mount, long before the
     * first scan.
     */
     ...
     * that is the three `.math` blocks (`overflow-x: auto` around 32-hex-char
     * field elements) and `.tageq` (`overflow-x: auto` around two rows of
     * sixteen tag cells).
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. Here that is the three
      // expert disclosures — the GF(2^128) sufficiency argument, the
      // partitioning-oracle note and the which-fixes-commit note — all of which
      // ship SHUT. The gate opens each by clicking its <summary>, which is the
      // route a reader has, rather than setting `.open` from script, which is
      // what the gate this replaces did to all three before its single scan.
      // `.math .step.hidden-step` (`display: none`) is caught by the plain
      // display test above.
      ...
      // header's `.cl-skip-link` — the page's only one — parks off the top of
      // the document and slides in only on focus. Measuring the parked copy invents a failure
      ...
      // `dom.ts`'s `srOnly()` emits one beside every abbreviated hex value on
      // the page — each key, each ciphertext block and the keystream pad — so a
      // reader who cannot see the `…` in the middle still gets all 32
      // characters. Every one of those spans has a rect and passes
      // `checkVisibility()` while painting exactly zero pixels; measuring them
      // would report a ratio for ink that was never laid down, and there is one
      // per block of every forgery. The guard is deliberately narrow: only a
     ...
     * `.verdict-icon` (⚠ / ✓ / ◆) and the shared header's two `<svg>` marks. In
     * every case the words that carry the meaning sit immediately beside the
     * glyph, in the same ink on the same surface — `.verdict-title` is the
     * sentence beside the ⚠ — so measuring the sibling measures the glyph too.
     * That pairing is the whole point of this lab's Constraint A: colour and
     * icon never carry a verdict alone.
     *
     * Nothing on this page hides a VALUE. No key, no tag, no ciphertext block,
     * no recovered plaintext, no verdict text and no fix result is inside an
     ...
     *
     * This lab has no SVG of its own — its two figures are `<canvas>` elements
     * painted from raw forged bytes, and the only SVG on the page is the shared
     * header's two `aria-hidden` marks. The guard stays because a walk that
     * measured a non-rendering SVG text node would report a 1:1 failure
     * describing ink that was never laid down, and the day this lab draws its
     * first real chart is the day that starts mattering.
     */
     ...
     *
     * Here it is currently load-bearing in the good direction: `style.css`
     * sets `html { background-color: var(--bg); color: var(--text) }`
     * explicitly. That declaration is one cleanup away from being deleted as
     * redundant, and this page runs eight panels deep — so the day it goes,
     * everything below the first screen would report against a white page that
     * does not exist, in a dark theme whose surfaces are all near-black.
     */
