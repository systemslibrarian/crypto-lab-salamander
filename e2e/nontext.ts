...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * individual `.tagcell` spans, twice, all re-walking the same ancestors up
     * to `<body>`. Without the caches the pass re-reads the same computed
     * styles and rects thousands of times. Nothing mutates the DOM during the
     ...
     * bright cyan control at 3.95:1 against a page that is actually near-black.
     * The tell was that the number came out IDENTICAL in both themes — a ratio
     * that cannot depend on the theme is not measuring the theme. This lab
     * paints flat `var(--bg)` on both `html` and `body` and declares no
     * gradient anywhere in `style.css`, so the sampler is a no-op here today;
     * it is kept
     * because it borrows `contrast.ts`'s model wholesale rather than
     * approximating it a second time, and the two files must not disagree about
     * what a surface is.
     */
